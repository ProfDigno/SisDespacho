...
public class BO_tercero_ciudad {
    ...
    public void update_tercero_ciudad_eliminar(tercero_ciudad teciu, JTable tbltabla) {
        if (evmen.MensajeGeneral_warning("ESTAS SEGURO DE ELIMINAR TERCERO_CIUDAD", "ELIMINAR", "ACEPTAR", "CANCELAR")) {
            String titulo = "update_tercero_ciudad";
            Connection conn = ConnPostgres.getConnPosgres();
            try {
                if (conn.getAutoCommit()) {
                ...
                teciu_dao.update_tercero_ciudad_eliminar(conn, teciu);
                teciu_dao.actualizar_tabla_tercero_ciudad(conn, tbltabla);
                conn.commit();
            } catch (SQLException e) {
                evmen.mensaje_error(e, teciu.toString(), titulo);
