...
//    private static String tk_monto = "15.000";
    ...
    private void cargar_datos_caja_cierre(Connection conn, int idcaja_cierre) {
        ...
        String sql = "select cc.idcaja_cierre,\n"
            + "to_char(cc.fecha_apertura,'yyyy-MM-dd HH24:MI') as inicio,\n"
            + "to_char(cc.fecha_cierre,'yyyy-MM-dd HH24:MI') as fin,\n"
            + "TRIM(to_char(sum(cd.monto_abrir),'999G999G999')) as in_Abrir,\n"
            + "TRIM(to_char(sum(cd.total_venta_efectivo),'999G999G999')) as in_venta,\n"
            + "TRIM(to_char(sum(cd.total_compra),'999G999G999')) as eg_compra,\n"
            + "TRIM(to_char(sum(cd.total_gasto),'999G999G999')) as eg_gasto,\n"
            + "TRIM(to_char(sum(cd.total_salario),'999G999G999')) as eg_vale,\n"
            + "TRIM(to_char((sum(cd.monto_abrir+cd.total_venta_efectivo))-(sum(cd.total_compra+cd.total_gasto+cd.total_salario)),'999G999G999')) as sistema,\n"
            + "TRIM(to_char(sum(cd.monto_cerrar),'999G999G999')) as cierre,\n"
            + "TRIM(to_char((sum(cd.monto_cerrar)-((sum(cd.monto_abrir+cd.total_venta_efectivo))-"
                + "(sum(cd.total_compra+cd.total_gasto+cd.total_salario)))),'999G999G999')) as diferencia\n"
